...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: HeliCoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: helicecoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizehelicecoinamount=<n> " + _("Keep N HeliCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. HeliCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. HeliCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("HeliCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "HeliCoin server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of HeliCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart HeliCoin to complete") << "\n";
    ...
    nAnonymizeHeliCoinAmount = GetArg("-anonymizehelicecoinamount", 0);
    if(nAnonymizeHeliCoinAmount > 999999) nAnonymizeHeliCoinAmount = 999999;
    if(nAnonymizeHeliCoinAmount < 2) nAnonymizeHeliCoinAmount = 2;
    ...
    LogPrintf("Anonymize HeliCoin Amount %d\n", nAnonymizeHeliCoinAmount);
