...
int nAnonymizeHeliCoinAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\HeliCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\HeliCoin
    // Mac: ~/Library/Application Support/HeliCoin
    // Unix: ~/.HeliCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HeliCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "HeliCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "helicecoind.pid"));
